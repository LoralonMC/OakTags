...
public class ItemsAdderProvider implements ModelProvider {
    ...
    public boolean applyModel(ItemStack item, String modelId) {
        try {
            ...
            if (customStack == null) {
                logger.warning("[OakTags] ItemsAdder item '" + modelId + "' not found. " +
            ...
            if (floats.isEmpty()) {
                logger.warning("[OakTags] ItemsAdder item '" + modelId + "' has no Item Model or CustomModelData. " +
            ...
            logger.warning("[OakTags] Failed to apply ItemsAdder model '" + modelId + "': " + e.getMessage());
            return false;
        }
    }
}
