    ...
    public boolean applyModel(ItemStack item, String modelId) {
        try {
            ...
            if (nexoItemBuilder == null) {
                logger.warning("[OakTags] Nexo item '" + modelId + "' not found. " +
            ...
            if (nexoItem == null || !nexoItem.hasItemMeta()) {
                logger.warning("[OakTags] Nexo item '" + modelId + "' has no metadata.");
                return false;
            }
            ...
            if (floats.isEmpty()) {
                logger.warning("[OakTags] Nexo item '" + modelId + "' has no Item Model or CustomModelData. " +
            ...
            logger.warning("[OakTags] Failed to apply Nexo model '" + modelId + "': " + e.getMessage());
            return false;
        }
    }
}
